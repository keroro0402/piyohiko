...
* */
    ...
    public class TestGlobalExceptionController {
        ...
        public void throw400ValidationException() throws MethodArgumentNotValidException {
            ...
            // 本番コード（.getFieldErrors()）がメッセージを抽出できるように、作成した器にダミーのエラーをセットする。
            // ※ここで第3引数に書いた文字列が、本番ではフロント（Nuxt.js）に届くFormで指定したエラーメッセージの本体になる。
            bindingResult.addError(new org.springframework.validation.FieldError(
                    "dummyTarget", "dummyField", "400、リクエストが必須チェックバリデーションに弾かれました"
            ));
            bindingResult.addError(new org.springframework.validation.FieldError(
                    "dummyTarget", "dummyField", "400、リクエストが形式バリデーションに弾かれました"
            ));
            bindingResult.addError(new org.springframework.validation.FieldError(
                    "dummyTarget", "dummyField", "400、リクエストが文字数バリデーションに弾かれました"
            ));
